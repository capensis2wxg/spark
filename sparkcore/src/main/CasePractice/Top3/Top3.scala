...
/*
    @author wxg
    @date 2021/8/3-10:40
    @description  统计出每个省份每条广告的点击量
 */
object Top3 {
	import org.apache.spark.{SparkConf, SparkContext}
	def main(args: Array[String]): Unit = {
		import org.apache.spark.rdd.RDD
		val sparkConf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("rdd/__01map")
		...
		val newMapRDD: RDD[(String, (String, Int))] = reduceRDD.map {case ((prv, ad), sum) => (prv, (ad, sum))}
