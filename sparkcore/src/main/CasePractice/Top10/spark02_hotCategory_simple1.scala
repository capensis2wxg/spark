...
object spark02_hotCategory_simple1 {
	...
	def main(args: Array[String]): Unit = {
		...
		val rddClick: RDD[(String, (Int, Int, Int))] = clickCountRDD.map {case (cid, cnt) => (cid, (cnt, 0, 0))}
		val rddOrder: RDD[(String, (Int, Int, Int))] = orderCountRDD.map {case (cid, cnt) => (cid, (0, cnt, 0))}
		val rddPay: RDD[(String, (Int, Int, Int))] = payCountRDD.map {case (cid, cnt) => (cid, (0, 0, cnt))}
		//	6、将三个数据源合并在一起，统一进行聚合计算
		val sourceRDD: RDD[(String, (Int, Int, Int))] = rddClick.union(rddOrder).union(rddPay)
		val analysisRDD: RDD[(String, (Int, Int, Int))] = sourceRDD.reduceByKey((t1: (Int, Int, Int), t2: (Int, Int, Int)) => {(t1._1 + t2._1, t1._2 + t2._2, t1._3 + t2._3)})
